...
> **Where this fits:** this folder is the original, self-contained
> "learn how neural networks work" project. It has been preserved
> unchanged (only relocated) inside the larger `BirdStrikeGeo` repository.
> It uses **entirely synthetic, hand-designed data** and is not related to,
> and not validated against, any real FAA or Trektellen data used
> elsewhere in this repository. See the top-level `README.md` for the
> real-data-oriented `BirdStrikeGeo` system.
...
Run these from **inside this `synthetic_demo/` folder**:
